...
6. **Фаза C: глоссарий** — C1 ✓; C2 после F2  
7. **Фаза F: работа в группе** — F0 ✓; F1 ✓; F2.0 ✓ (manual smoke pending); F2.1  
8. Tag MVP  
...
### Фаза F: работа в группе ← **в MVP** (F0 ✓; F1 ✓; F2.0 ✓; F2.1)
...
- [x] Спека: [`docs/superpowers/specs/2026-07-17-group-collaboration-design.md`](docs/superpowers/specs/2026-07-17-group-collaboration-design.md)
- [x] Members, роли (owner / editor / viewer), invite-ссылки (TTL / maxUses / именные / revoke)
- [x] Project TM + attach с `canRead` / `canWrite` / `canExport` / `canClone`
- [x] Lock: F2.0 project → F2.1 segment + presence
- [x] Явно: email в audit/TM/presence не пишем; только ник / `anon:userId`

**F1 — минимальный обмен без полного collab** (быстрый wins) ✓

- [x] UX: «Поделиться проектом» → скачать `.tcat.zip` + короткая подсказка «откройте в другом аккаунте через импорт»
- [x] При импорте чужого проекта: сохранить `audit[].by` / TM actors как есть (Инкогнито+цвета / ники)
- [ ] (Опц.) экспортировать/импортировать только TMX с attribution notes

**F2.0 — cloud shared project** ✓ (manual smoke pending)

- [x] Таблица `project_members` (`project_id`, `user_id`, `role`)
- [x] Invite: создать invite → принять → member (TTL / maxUses / revoke)
- [x] Sync проекта (segments + meta; LWW; push только под project lock)
- [x] Project lock: один editor; viewer read-only
- [x] Presence heartbeats (ник / `anon:userId`; без email)
- [x] Project TM + attach ACL (`canRead` / `canWrite` / `canExport` / `canClone`)
- [x] UI: список участников, invites, accept route; в истории/ТМ «Вы» vs ник vs Инкогнито
- [ ] Manual smoke: два аккаунта — invite, lock, edit, match из project TM — **pending**

**F2.1 — segment locks + presence avatars**

- [ ] Segment/block lock на фокус; аватары на занятых сегментах
- [ ] Sync POST отклоняет обновление сегмента под чужим segment lock
...
`F0 → F1 → F2.0 → F2.1 → F3`. F1 ✓; F2.0 ✓ (manual smoke pending); дальше F2.1.
...
**C1 ✓:** личный termbase на user + IndexedDB + cloud sync (как TM) + подсветка в source + TBX + status approved/forbidden  

**C2 (после F2 / с members):** проектный termbase / attach — без ломки модели entry  

- [x] Спека C1 ✓ (файл выше)
- [x] Реализация C1
- [ ] C2 — project attach
