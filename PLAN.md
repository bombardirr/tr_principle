...
| **Штраф за теги** | Сейчас: разные `{1}…{2}` (маркеры форматирования runs) → понижаем % fuzzy. **Переименовать** после фазы «Стили»: в профессиональном CAT «теги» = другое (см. фазу E). |
...
Спека (когда начнём): `docs/superpowers/specs/YYYY-MM-DD-styles-vs-tags-design.md` — писать перед кодом.
...
- Переименовывать TM «штраф за теги» в UI до смены модели редактора (путаница); сначала словарь в плане, потом i18n.
...
4. **Панель стилей** — один раз в **шапке редактора**, действует на выделение в **активном** сегменте (не в каждом ряду).
...
- [ ] Спека: стили vs маркеры vs колонтитулы; что остаётся в MVP UX
- [ ] i18n: «маркеры форматирования» вместо «теги» где речь о `{n}`; kind header/footer не называть tags
- [ ] В словаре B2: сноска что «штраф за теги» = штраф за маркеры (временно)
...
**E2 — панель стилизации (шапка)**

- [ ] Toolbar в шапке: B / I / U (+ размер/шрифт — по возможности из доступных в сегменте)
...
- [ ] TM: переименовать penalty в UI на «маркеры форматирования» или отдельно penalty за structure tags
...
- [ ] **Ручная стилизация перевода** (фаза E) — вместо «ручная вставка тегов `{n}`»
