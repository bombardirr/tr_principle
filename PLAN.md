...
- [ ] **Одна активная сессия** — не допускать одновременную работу нескольких людей с одного аккаунта (или хотя бы одного проекта)
  - **Сейчас (без сервера, фаза A):** [x] мягкая блокировка в браузере — одна вкладка-«лидер» на проект (`BroadcastChannel` + lease в localStorage); вторая вкладка/окно: «Проект открыт в другой вкладке», только просмотр.
