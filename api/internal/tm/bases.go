...
const (
	personalBaseID = "personal-tm"
	defaultColor   = "#5b9fd4"
)

var (
	ErrBaseNotFound     = errors.New("base not found")
	ErrCannotDeleteBase = errors.New("personal-tm cannot be deleted")
...
func (s *Store) ListBases(ctx context.Context, ownerID uuid.UUID) ([]Base, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&base.ID, &base.Label, &base.Color, &createdAt, &updatedAt, &deletedAt,
		); err != nil {
			return nil, err
		}
		base.CreatedAt = formatTime(createdAt)
		base.UpdatedAt = formatTime(updatedAt)
		if deletedAt != nil {
...
func (s *Store) PatchBase(
	...
	`, ownerID, id, cleanLabel, cleanColor)
	if err != nil {
		return err
	}
