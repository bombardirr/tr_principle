...
func TestJobOriginalShare(t *testing.T) {
	...
	job := requestJSON(t, http.MethodGet, srv.URL+"/api/projects/"+jobID.String(), ownerToken, nil, http.StatusOK)
	...
	requestRaw(t, http.MethodDelete, srv.URL+"/api/projects/"+jobID.String()+"/members/me", translatorToken, nil, http.StatusNoContent)
	getOriginal(t, srv.URL, jobID, ownerToken, http.StatusOK)

	requestRaw(t, http.MethodDelete, srv.URL+"/api/projects/"+jobID.String()+"/original", ownerToken, nil, http.StatusNoContent)
	...
	requestRaw(t, http.MethodDelete, srv.URL+"/api/projects/"+jobID.String()+"/original", ownerToken, nil, http.StatusNoContent)
	requestJSON(t, http.MethodPost, srv.URL+"/api/projects/"+jobID.String()+"/archive", ownerToken, nil, http.StatusOK)
	archivedDelete := requestRaw(t, http.MethodDelete, srv.URL+"/api/projects/"+jobID.String()+"/original", ownerToken, nil, http.StatusBadRequest)
	...
	requestRaw(t, http.MethodDelete, srv.URL+"/api/projects/"+deleteJobID.String(), ownerToken, nil, http.StatusNoContent)
	...
	sameHash := requestJSON(t, http.MethodPatch, srv.URL+"/api/projects/"+revokeOriginalID.String(), ownerToken, map[string]any{
		"sourceHash": hash,
	}, http.StatusOK)
	...
	noHash := requestJSON(t, http.MethodPatch, srv.URL+"/api/projects/"+revokeOriginalID.String(), ownerToken, map[string]any{
	...
	changedHash := requestJSON(t, http.MethodPatch, srv.URL+"/api/projects/"+revokeOriginalID.String(), ownerToken, map[string]any{
...
func acceptInviteForRole(t *testing.T, baseURL string, jobID uuid.UUID, ownerToken, memberToken, role string) {
	t.Helper()
	invite := requestJSON(t, http.MethodPost, baseURL+"/api/projects/"+jobID.String()+"/invites", ownerToken, map[string]any{
		"role": role,
	}, http.StatusCreated)
...
func putOriginal(t *testing.T, baseURL string, jobID uuid.UUID, token string, body []byte, filename string, wantStatus int) []byte {
	t.Helper()
	req, err := http.NewRequest(http.MethodPut, baseURL+"/api/projects/"+jobID.String()+"/original", bytes.NewReader(body))
	if err != nil {
		t.Fatal(err)
	}
...
func getOriginal(t *testing.T, baseURL string, jobID uuid.UUID, token string, wantStatus int) []byte {
	t.Helper()
	req, err := http.NewRequest(http.MethodGet, baseURL+"/api/projects/"+jobID.String()+"/original", nil)
	if err != nil {
		t.Fatal(err)
	}
	req.Header.Set("Authorization", "Bearer "+token)
...
func headOriginal(t *testing.T, baseURL string, jobID uuid.UUID, token string, wantStatus int) {
	t.Helper()
	req, err := http.NewRequest(http.MethodHead, baseURL+"/api/projects/"+jobID.String()+"/original", nil)
	if err != nil {
		t.Fatal(err)
	}
	req.Header.Set("Authorization", "Bearer "+token)
