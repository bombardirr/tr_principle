...
const resourceCache = new Map<string, CloudProjectResource | null>()
...
export function cacheJobResource(jobId: string, resource: CloudProjectResource | null): void {
  resourceCache.set(jobId, resource)
  if (typeof localStorage === 'undefined') return
  localStorage.setItem(resourceCacheKey(jobId), JSON.stringify(resource))
}

export function readCachedJobResource(jobId: string): CloudProjectResource | null {
  ...
  try {
    ...
    if (!raw) return null
    const resource = JSON.parse(raw) as CloudProjectResource | null
    resourceCache.set(jobId, resource)
    return resource
  } catch {
    return null
...
export async function loadJobResource(jobId: string): Promise<CloudProjectResource | null> {
  try {
...
export function jobTmReadable(resource: CloudProjectResource | null | undefined): boolean {
  return Boolean(resource?.enabled && resource.canRead)
}

export function jobTmWritable(resource: CloudProjectResource | null | undefined): boolean {
